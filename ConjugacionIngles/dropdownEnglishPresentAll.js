...
function setBoard() {
    ...
    for (var i = 0; i < arr.length; i++) {
        number = i + 1;
        text += "<span class='numbers'>" + number + ".</span> " + pronounsquestions[i] + " <select id='dropdown" + [i] + "' class='dropdown'>" + optionslist[i][0] + optionslist[i][1] + optionslist[i][2] + "</select></p>"
    };
...
function answer() {
    ...
    if (reset == false) {
        for (var i = 0; i < arr.length; i++) {
            number = i + 1;
            correctText += "<span class='numbers'>" + number + ".</span> " + pronounsquestions[i] + " <span id='answer" + i + "' class=''>" + anItemsCorrect[i] + "</span></p>";
        };
