...
for (var i = 0; i < ejercicios; i++) {
  ...
  var numero = i + 1;
  reactivo = '<p class="reactivo">' + numero + '. </p>' + '<img src="img/' + numero + '.png"><br><p class="instrucciones">Selecciona los n&uacute;meros cu&aacute;nticos</p><br>' + table + '</p><br>';
...
function answer() {
  ...
  for (var i = 0; i < ejercicios; i++) {
    ...
    table = '<table>' + row + '</table>';
    var numero = i + 1;
    respuesta = '<p class="reactivo">' + numero + '. </p>' + '<img src="juegos.aprendecontabella.com/dropdownImagen/img/numerocuantico' + numero + '.png"><br>' + table + '</p><br>';
  ...
  for (var i = 0; i < dropdownNumber; i++) {
    ...
    if (answers[i] == flattened[i]) {
      ...
      document.getElementById(numeroRespuesta).classList.add("incorrect"); //document.getElementById(numeroRespuesta).innerHTML += "<span id='answer" + i + "' class='correct'> <span class='incorrect strike'>" + anItems[answers[i]] + "</span> " + anItems[anItemsCorrect[i]] + "</span>";
    }
  }
